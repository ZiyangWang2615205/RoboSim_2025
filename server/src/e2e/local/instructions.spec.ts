...
test("can switch from Submission-Format to FAQ", async ({ page, account: _ }) => {
    await page.goto("/instructions/");
...
test("can load Submission-Format directly from url query", async ({ page, account: _ }) => {
...
test("can load FAQ directly from url query", async ({ page, account: _ }) => {
    await page.goto("/instructions/?doc=faq");
     //check heading
    await expect(
        page.getByRole("heading", { name: "Algorithm Client Instructions" })
    ).toBeVisible();
...
test("shows error message when markdown document fails to load", async ({ page, account: _ }) => {
    //create an intercept
    await page.route("/docs/FAQ.md", async (route) => {
        await route.fulfill({
            status: 500,
    ...
    await expect(page.locator("#doc-content")).toContainText(
